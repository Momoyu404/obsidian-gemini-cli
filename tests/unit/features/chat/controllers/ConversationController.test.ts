...
describe('ConversationController - Rewind', () => {
  ...
  it('should fallback to local file restoration when native rewind reports no file changes', async () => {
    const adapter = {
      basePath: '/mock/vault/path',
      write: jest.fn().mockResolvedValue(undefined),
      exists: jest.fn().mockResolvedValue(true),
      read: jest.fn().mockResolvedValue('current body'),
    };
        ...
        toolCalls: [
          {
            id: 'tool-1',
            name: TOOL_WRITE,
            input: { file_path: '/mock/vault/path/note.md', _snapshotPath: 'note.md' },
            status: 'completed',
            fileSnapshotBefore: 'old body',
          },
        ],
      },
    ];
    mockAgentService.rewind.mockResolvedValue({ canRewind: true, filesChanged: [] });

    await controller.rewind('m2');

    expect(mockAgentService.rewind).toHaveBeenCalledWith('user-uuid', 'prev-a');
    expect(adapter.write).toHaveBeenCalledWith('note.md', 'old body');
    expect(deleteFile).not.toHaveBeenCalled();
    const noticeMsg = mockNotice.mock.calls[mockNotice.mock.calls.length - 1][0] as string;
    expect(noticeMsg).toContain('1 file');
  });

  it('should restore files locally during rewindLast using tracked edit operations', async () => {
    const adapter = {
      basePath: '/mock/vault/path',
      write: jest.fn().mockResolvedValue(undefined),
      exists: jest.fn().mockResolvedValue(true),
      read: jest.fn().mockResolvedValue('new body and another new body'),
    };
        ...
        toolCalls: [
          {
            ...
            name: TOOL_EDIT,
            input: {
              file_path: '/mock/vault/path/note.md',
              _snapshotPath: 'note.md',
              old_string: 'old body',
              new_string: 'new body',
            },
            status: 'completed',
    ...
    await controller.rewindLast();

    expect(mockAgentService.setSessionId).toHaveBeenCalledWith(null);
    expect(adapter.read).toHaveBeenCalledWith('note.md');
    expect(adapter.write).toHaveBeenCalledWith('note.md', 'old body and another old body');
    const noticeMsg = mockNotice.mock.calls[mockNotice.mock.calls.length - 1][0] as string;
    expect(noticeMsg).toContain('1 file');
  });
});
