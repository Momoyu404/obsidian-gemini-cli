  ...
  describe('handleApprovalRequest', () => {
    ...
    it.each([
      ...
      const target = items.find((item: any) => {
        const label = item.querySelector('geminese-ask-item-label');
    ...
    it('should render header metadata when approvalOptions provided', async () => {
      ...
      const reasonEl = parentEl.querySelector('geminese-ask-approval-reason');
      expect(reasonEl?.textContent).toBe('Command is destructive');

      const pathEl = parentEl.querySelector('geminese-ask-approval-blocked-path');
      expect(pathEl?.textContent).toBe('/usr/bin/rm');

      const agentEl = parentEl.querySelector('geminese-ask-approval-agent');
