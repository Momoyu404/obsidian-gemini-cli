...
describe('sdkSession', () => {
  ...
  describe('findLatestFileHistorySnapshot', () => {
    it('returns the latest snapshot state for a rewound user message', () => {
      ...
      expect(findLatestFileHistorySnapshot(entries, 'user-1')).toEqual({
        messageId: 'user-1',
        trackedFileBackups: {
          'notes/test.md': {
            backupFileName: 'backup@v1',
    ...
    it('returns null when no snapshot exists for the target user message', () => {
      const entries: SDKNativeMessage[] = [
        {
          type: 'file-history-snapshot',
          messageId: 'user-2',
      ...
      expect(findLatestFileHistorySnapshot(entries, 'user-1')).toBeNull();
    });
  });
