...
describe('GeminianService', () => {
  ...
  describe('rewind', () => {
    it('restores tracked vault files from SDK file history before completing rewind', async () => {
      service.setSessionId('session-123');
      jest.spyOn(sdkSessionUtils, 'readSDKSession').mockResolvedValue({
        skippedLines: 0,
        messages: [
          {
            ...
            snapshot: {
              ...
              trackedFileBackups: {
                'notes/test.md': {
                  ...
                  version: 1,
                  backupTime: '2026-03-12T00:00:01.000Z',
                },
              },
      ...
      const readFileSpy = jest.spyOn(fs.promises, 'readFile').mockResolvedValue('original note');
      ...
      expect(readFileSpy.mock.calls[0][0]).toBe(
        path.join(process.env.HOME || '', '.claude', 'file-history', 'session-123', 'backup@v1')
      );
      expect(readFileSpy.mock.calls[0][1]).toBe('utf-8');
      expect(mkdirSpy).toHaveBeenCalledWith('/mock/vault/path/notes', { recursive: true });
      expect(writeFileSpy).toHaveBeenCalledWith(
        '/mock/vault/path/notes/test.md',
        'original note',
        'utf-8'
      );
      expect(result).toEqual({ canRewind: true, filesChanged: ['notes/test.md'] });
    });
