...
export class McpTestModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('geminese-mcp-test-modal');
  ...
  private renderLoading() {
    ...
    const loadingEl = this.contentEl_.createDiv({ cls: 'geminese-mcp-test-loading' });

    const spinnerEl = loadingEl.createDiv({ cls: 'geminese-mcp-test-spinner' });
    const spinnerSvg = new DOMParser().parseFromString(
      `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>`,
      'image/svg+xml'
    ).documentElement;
    spinnerEl.appendChild(document.adoptNode(spinnerSvg));
  ...
  private render() {
    ...
    const statusEl = this.contentEl_.createDiv({ cls: 'geminese-mcp-test-status' });

    const iconEl = statusEl.createSpan({ cls: 'geminese-mcp-test-icon' });
    ...
    const textEl = statusEl.createSpan({ cls: 'geminese-mcp-test-text' });
    if (this.result.success) {
    ...
    if (this.result.error) {
      const errorEl = this.contentEl_.createDiv({ cls: 'geminese-mcp-test-error' });
    ...
    if (this.result.tools.length > 0) {
      const toolsSection = this.contentEl_.createDiv({ cls: 'geminese-mcp-test-tools' });

      const toolsHeader = toolsSection.createDiv({ cls: 'geminese-mcp-test-tools-header' });
      toolsHeader.setText(`Available Tools (${this.result.tools.length})`);

      const toolsList = toolsSection.createDiv({ cls: 'geminese-mcp-test-tools-list' });

      for (const tool of this.result.tools) {
        this.renderTool(toolsList, tool);
      }
    } else if (this.result.success) {
      const noToolsEl = this.contentEl_.createDiv({ cls: 'geminese-mcp-test-no-tools' });
      noToolsEl.setText('No tools information available. Tools will be loaded when used in chat.');
    }

    const buttonContainer = this.contentEl_.createDiv({ cls: 'geminese-mcp-test-buttons' });
  ...
  private renderTool(container: HTMLElement, tool: McpTool) {
    const toolEl = container.createDiv({ cls: 'geminese-mcp-test-tool' });

    const headerEl = toolEl.createDiv({ cls: 'geminese-mcp-test-tool-header' });

    const iconEl = headerEl.createSpan({ cls: 'geminese-mcp-test-tool-icon' });
    setIcon(iconEl, 'wrench');

    const nameEl = headerEl.createSpan({ cls: 'geminese-mcp-test-tool-name' });
    nameEl.setText(tool.name);

    const toggleEl = headerEl.createDiv({ cls: 'geminese-mcp-test-tool-toggle' });
    ...
    if (tool.description) {
      const descEl = toolEl.createDiv({ cls: 'geminese-mcp-test-tool-desc' });
  ...
  private updateToolState(toolEl: HTMLElement, enabled: boolean) {
    toolEl.toggleClass('geminese-mcp-test-tool-disabled', !enabled);
  }
