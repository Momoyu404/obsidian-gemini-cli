...
Main sidebar chat interface. `GemineseView` is a thin shell; logic lives in controllers and services.

## Architecture

```
GemineseView (lifecycle + assembly)
...
```
User Input → InputController → GemineseService.query()
                                      ↓
...
```typescript
// GemineseService created on first query, not on tab create
...
- `GemineseView.onClose()` must abort all tabs and dispose services
