...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'geminese-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'geminese-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'geminese-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'geminese-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.geminese-thinking-header');
  if (header) {
