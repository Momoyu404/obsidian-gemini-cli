...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `obsidian-gemini-message obsidian-gemini-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'obsidian-gemini-message-content', attr: { dir: 'auto' } });
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'obsidian-gemini-message obsidian-gemini-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'obsidian-gemini-message-content', attr: { dir: 'auto' } });
    const textEl = contentEl.createDiv({ cls: 'obsidian-gemini-text-block' });
    textEl.innerHTML = '<span class="obsidian-gemini-interrupted">Interrupted</span> <span class="obsidian-gemini-interrupted-hint">· What should Geminian do instead?</span>';
  }
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'obsidian-gemini-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'obsidian-gemini-message-image' });
  ...
  async renderContent(el: HTMLElement, markdown: string): Promise<void> {
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('obsidian-gemini-code-wrapper')) return;

        // Create wrapper
        const wrapper = createEl('div', { cls: 'obsidian-gemini-code-wrapper' });
        ...
        if (code) {
          ...
          if (match) {
            ...
            const label = createEl('span', {
              cls: 'obsidian-gemini-code-lang-label',
              text: match[1],
            });
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'obsidian-gemini-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (this.forkCallback && !msgEl.querySelector('.obsidian-gemini-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(msgId: string, msgEl: HTMLElement): void {
    const needsFork = this.forkCallback && !msgEl.querySelector('.obsidian-gemini-message-fork-btn');
    if (!needsFork) {
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector('.obsidian-gemini-user-msg-actions') as HTMLElement | null;
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'obsidian-gemini-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'obsidian-gemini-user-msg-copy-btn' });
    copyBtn.innerHTML = MessageRenderer.COPY_ICON;
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'obsidian-gemini-message-fork-btn' });
