...
function createSection(parentEl: HTMLElement, title: string, bodyClass?: string): SubagentSection {
  const wrapperEl = parentEl.createDiv({ cls: 'obsidian-gemini-subagent-section' });

  const headerEl = wrapperEl.createDiv({ cls: 'obsidian-gemini-subagent-section-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const titleEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-section-title' });
  titleEl.setText(title);

  const bodyEl = wrapperEl.createDiv({ cls: 'obsidian-gemini-subagent-section-body' });
...
function setPromptText(promptBodyEl: HTMLElement, prompt: string): void {
  promptBodyEl.empty();
  const textEl = promptBodyEl.createDiv({ cls: 'obsidian-gemini-subagent-prompt-text' });
...
function renderSubagentToolContent(contentEl: HTMLElement, toolCall: ToolCallInfo): void {
  ...
  if (!toolCall.result) {
    const emptyEl = contentEl.createDiv({ cls: 'obsidian-gemini-subagent-tool-empty' });
...
function setSubagentToolStatus(view: SubagentToolView, status: ToolCallInfo['status']): void {
  view.statusEl.className = 'obsidian-gemini-subagent-tool-status';
...
function updateSubagentToolView(view: SubagentToolView, toolCall: ToolCallInfo): void {
  view.wrapperEl.className = `obsidian-gemini-subagent-tool-item obsidian-gemini-subagent-tool-${toolCall.status}`;
...
function createSubagentToolView(parentEl: HTMLElement, toolCall: ToolCallInfo): SubagentToolView {
  const wrapperEl = parentEl.createDiv({
    cls: `obsidian-gemini-subagent-tool-item obsidian-gemini-subagent-tool-${toolCall.status}`,
  });
  wrapperEl.dataset.toolId = toolCall.id;

  const headerEl = wrapperEl.createDiv({ cls: 'obsidian-gemini-subagent-tool-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-tool-name' });
  const summaryEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-tool-summary' });
  const statusEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-tool-status' });

  const contentEl = wrapperEl.createDiv({ cls: 'obsidian-gemini-subagent-tool-content' });
...
function ensureResultSection(state: SubagentState): SubagentSection {
  ...
  const section = createSection(state.contentEl, 'Result', 'obsidian-gemini-subagent-result-body');
  section.wrapperEl.addClass('obsidian-gemini-subagent-section-result');
...
function setResultText(state: SubagentState, text: string): void {
  ...
  const resultEl = section.bodyEl.createDiv({ cls: 'obsidian-gemini-subagent-result-output' });
...
function hydrateSyncSubagentStateFromStored(state: SubagentState, subagent: SubagentInfo): void {
  ...
  if (subagent.status === 'completed' || subagent.status === 'error') {
    ...
    state.statusEl.className = 'obsidian-gemini-subagent-status status-running';
...
export function createSubagentBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'obsidian-gemini-subagent-list' });
  wrapperEl.dataset.subagentId = taskToolId;

  const headerEl = wrapperEl.createDiv({ cls: 'obsidian-gemini-subagent-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(TOOL_TASK));

  const labelEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-label' });
  labelEl.setText(truncateDescription(description));

  const countEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-count' });
  countEl.setText('0 tool uses');

  const statusEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  const contentEl = wrapperEl.createDiv({ cls: 'obsidian-gemini-subagent-content' });

  const promptSection = createSection(contentEl, 'Prompt', 'obsidian-gemini-subagent-prompt-body');
  promptSection.wrapperEl.addClass('obsidian-gemini-subagent-section-prompt');
  setPromptText(promptSection.bodyEl, prompt);

  const toolsContainerEl = contentEl.createDiv({ cls: 'obsidian-gemini-subagent-tools' });
...
export function finalizeSubagentBlock(
  ...
  state.statusEl.className = 'obsidian-gemini-subagent-status';
...
function renderAsyncContentLikeSync(
  ...
  const resultSection = createSection(contentEl, 'Result', 'obsidian-gemini-subagent-result-body');
  resultSection.wrapperEl.addClass('obsidian-gemini-subagent-section-result');
  const resultEl = resultSection.bodyEl.createDiv({ cls: 'obsidian-gemini-subagent-result-output' });
...
export function createAsyncSubagentBlock(
  ...
  const statusTextEl = headerEl.createDiv({ cls: 'obsidian-gemini-subagent-status-text' });
...
export function markAsyncSubagentOrphaned(state: AsyncSubagentState): void {
  ...
  state.statusEl.className = 'obsidian-gemini-subagent-status status-error';
  state.statusEl.empty();
...
export function renderStoredAsyncSubagent(
  ...
  const statusEl = headerEl.createDiv({ cls: `obsidian-gemini-subagent-status ${statusIconClass}` });
