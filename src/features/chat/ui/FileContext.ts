...
export class FileContextManager {
  ...
  async handleFileDrop(e: DragEvent): Promise<void> {
    ...
    // 1. Try Obsidian internal drag
    const obsDnD = dataTransfer.getData('application/x-obsidian-dnd');
    if (obsDnD || dataTransfer.types.includes('application/x-obsidian-dnd')) {
      try {
        const payloadStr = obsDnD || dataTransfer.getData('text/plain');
        if (payloadStr) {
          const payload = JSON.parse(payloadStr);
          if (payload.type === 'file' && typeof payload.file === 'string') {
            this.state.attachFile(payload.file);
            attached = true;
          } else if (payload.type === 'files' && Array.isArray(payload.files)) {
    ...
    // 2. Fallback to OS file drop
    if (!attached && dataTransfer.files && dataTransfer.files.length > 0) {
      for (let i = 0; i < dataTransfer.files.length; i++) {
        const file = dataTransfer.files.item(i);
        if (!file) continue;

        // Image files handled by ImageContextManager, skip them here
