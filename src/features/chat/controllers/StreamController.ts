  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        // Snapshot file if file_path just became available in this update chunk
        if (isWriteEditTool(existingToolCall.name) && existingToolCall.fileSnapshotBefore === undefined && existingToolCall.input?.file_path) {
          void this.snapshotFileBeforeWrite(existingToolCall);
        }
    ...
    // Fire-and-forget snapshot: capture file content before Gemini CLI executes Write/Edit.
    // tool_use arrives before the CLI runs the tool, so reading here gets the pre-write state.
    if (isWriteEditTool(chunk.name) && chunk.input?.file_path) {
      void this.snapshotFileBeforeWrite(toolCall);
    }
  }

  /**
   * Reads the vault file at toolCall.input.file_path and stores its content
   * in toolCall.fileSnapshotBefore so rewindLast() can restore it later.
   * Called as fire-and-forget when a Write/Edit tool_use event arrives.
   */
  private async snapshotFileBeforeWrite(toolCall: ToolCallInfo): Promise<void> {
    if (toolCall.fileSnapshotBefore !== undefined) return; // already snapshotted
    const filePath = toolCall.input?.file_path as string | undefined;
    if (!filePath) return;
    try {
      // Gemini CLI provides absolute paths; vault.adapter requires vault-relative paths
      const vaultPath = getVaultPath(this.deps.plugin.app);
      const relativePath = normalizePathForVault(filePath, vaultPath);
      if (!relativePath) return;
      toolCall.input = { ...toolCall.input, _snapshotPath: relativePath }; // cache normalized path
      const adapter = this.deps.plugin.app.vault.adapter;
      const exists = await adapter.exists(relativePath);
      toolCall.fileSnapshotBefore = exists ? await adapter.read(relativePath) : null;
    } catch {
      // Best-effort — leave undefined so rewind skips this file
    }
  }
