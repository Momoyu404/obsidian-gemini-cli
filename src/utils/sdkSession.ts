...
export interface SDKNativeMessage {
  ...
  content?: string;
  /** File history snapshot payload written by the SDK before/after tool edits. */
  snapshot?: SDKFileHistorySnapshot;
  /** True when the SDK emits an updated snapshot after a file edit completes. */
  isSnapshotUpdate?: boolean;
}

export interface SDKTrackedFileBackup {
  backupFileName: string | null;
  version: number;
  backupTime: string;
}

export interface SDKFileHistorySnapshot {
  messageId: string;
  trackedFileBackups: Record<string, SDKTrackedFileBackup>;
  timestamp: string;
}
...
export function getSDKFileHistoryPath(sessionId: string, backupFileName?: string): string {
  if (!isValidSessionId(sessionId)) {
    throw new Error(`Invalid session ID: ${sessionId}`);
  }

  const basePath = path.join(os.homedir(), '.claude', 'file-history', sessionId);
  return backupFileName ? path.join(basePath, backupFileName) : basePath;
}
...
export function findLatestFileHistorySnapshot(
  entries: SDKNativeMessage[],
  userMessageId: string
): SDKFileHistorySnapshot | null {
  let latestSnapshot: SDKFileHistorySnapshot | null = null;

  for (const entry of entries) {
    if (entry.type !== 'file-history-snapshot' || !entry.snapshot) continue;

    const snapshotMessageId =
      typeof entry.snapshot.messageId === 'string' && entry.snapshot.messageId.length > 0
        ? entry.snapshot.messageId
        : entry.messageId;

    if (snapshotMessageId !== userMessageId) continue;
