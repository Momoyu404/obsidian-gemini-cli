...
├── settings/       # base (shared .geminese-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: All classes use `.geminese-` prefix
- **BEM-lite**: `.geminese-{block}`, `.geminese-{block}-{element}`, `.geminese-{block}--{modifier}`
